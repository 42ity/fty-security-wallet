...
std::map<DocumentType, FctDocumentFactory> Document::m_documentFactoryFuntions =
{
    ...
    { TOKEN_AND_LOGIN_TYPE, []() { return DocumentPtr(new TokenAndLogin()); }},
    { SSHKEY_AND_LOGIN_TYPE, []() { return DocumentPtr(new SshKeyAndLogin()); }},
...
const Id& Document::getId() const
{
    return m_id;
}

bool Document::isContainingPrivateData() const
{
    return m_containPrivateData;
}
...
bool Document::isSecretEquals(const DocumentPtr& other) const
{
    if (m_type != other->getType())
        return false;
    if (!m_containPrivateData || !other->isContainingPrivateData())
        return false;
