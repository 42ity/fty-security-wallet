/*  =========================================================================
    fty_security_wallet_selftest.c - run selftests
...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_SECURITY_WALLET_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_security_wallet_private_selftest (verbose, item->subtest);
#endif // FTY_SECURITY_WALLET_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_security_wallet_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-security-wallet test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_SECURITY_WALLET_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_security_wallet_private_selftest (verbose, test->subtest);
#endif // FTY_SECURITY_WALLET_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
